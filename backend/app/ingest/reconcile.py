...
# Every interpolated path below is resolved from the caller and escaped before
# use; the builder carries a scoped lint suppression.
...
def _build_load_sql(shard_dir: str) -> list[tuple[str, str | None]]:
...
""",
    ...
    if "records" in tables_with_data:
        statements.append(("DROP TABLE IF EXISTS record_id_mapping", None))
    if "workouts" in tables_with_data:
        statements.append(("DROP TABLE IF EXISTS workout_id_mapping", None))
...
def load_shards_into_duckdb(db: duckdb.DuckDBPyConnection, shard_dir: str | Path) -> None:
    ...
    logger.debug(
        "reconcile.shards.sample", extra={"payload": {"count": min(5, len(parquet_files))}}
    )
